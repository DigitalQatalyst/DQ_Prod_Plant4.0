-- ============================================================================
-- APM Transmission FS5: apm_alerts, Reports & Visualisation - Validation Script
-- ============================================================================
-- Feature Set: FS5 - apm_alerts, Reports & Visualisation
...
-- Purpose: Ensure all apm_alerts have valid severity and source values
-- Requirements: 20.2
-- Expected: 0 rows (all apm_alerts have valid values)
-- ============================================================================
...
BEGIN
  ...
  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'Validation failed: % apm_alerts have invalid severity or source values', invalid_count;
  ELSE
    RAISE NOTICE '✓ Validation 1 passed: All apm_alerts have valid severity and source values';
  END IF;
END $$;

-- ============================================================================
-- Validation 2: Alert Closure Notes Required
-- Purpose: Ensure closed apm_alerts have resolution_notes
-- Requirements: 20.8
-- Expected: 0 rows (all closed apm_alerts have resolution notes)
-- ============================================================================
...
BEGIN
  ...
  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'Validation failed: % closed apm_alerts missing resolution_notes', invalid_count;
  ELSE
    RAISE NOTICE '✓ Validation 2 passed: All closed apm_alerts have resolution notes';
  END IF;
END $$;

-- ============================================================================
-- Validation 3: Alert State Transition Consistency
-- Purpose: Ensure acknowledged apm_alerts have acknowledged_by and acknowledged_at
-- Requirements: 20.7
-- Expected: 0 rows (all acknowledged apm_alerts have required fields)
-- ============================================================================
...
BEGIN
  ...
  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'Validation failed: % acknowledged apm_alerts missing acknowledged_by or acknowledged_at', invalid_count;
  ELSE
    RAISE NOTICE '✓ Validation 3 passed: All acknowledged apm_alerts have required fields';
  END IF;
END $$;

-- ============================================================================
-- Validation 4: Alert History References Valid apm_alerts
-- Purpose: Ensure all apm_alert_history records reference existing apm_alerts
-- Requirements: 21.4, 21.5
-- Expected: 0 rows (all history records link to valid apm_alerts)
-- ============================================================================
...
BEGIN
  SELECT COUNT(*)
  INTO invalid_count
  FROM apm_alert_history ah
  WHERE NOT EXISTS (
    SELECT 1 FROM apm_alerts a WHERE a.id = ah.alert_id
  );
  
  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'Validation failed: % apm_alert_history records reference non-existent apm_alerts', invalid_count;
  ELSE
    RAISE NOTICE '✓ Validation 4 passed: All alert history records reference valid apm_alerts';
  END IF;
END $$;

-- ============================================================================
-- Validation 4.1: Alert State Changes Have Corresponding History Records
-- Purpose: Ensure apm_alerts in 'ack' or 'closed' state have corresponding history records
-- Requirements: 21.5
...
DECLARE
  invalid_count INTEGER;
BEGIN
  -- Check apm_alerts in 'ack' state have history record showing transition to 'ack'
  SELECT COUNT(*)
  INTO invalid_count
  FROM apm_alerts a
  WHERE a.state = 'ack'
    AND NOT EXISTS (
      SELECT 1 FROM apm_alert_history ah 
      WHERE ah.alert_id = a.id 
        AND ah.new_state = 'ack'
    );
  
  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'Validation failed: % acknowledged apm_alerts missing history records for ack transition', invalid_count;
  END IF;
  
  -- Check apm_alerts in 'closed' state have history record showing transition to 'closed'
  SELECT COUNT(*)
  INTO invalid_count
  FROM apm_alerts a
  ...
  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'Validation failed: % closed apm_alerts missing history records for closed transition', invalid_count;
  ELSE
...
-- Purpose: Ensure all apm_alerts reference existing assets
-- Requirements: 20.3
-- Expected: 0 rows (all apm_alerts link to valid assets)
-- ============================================================================
...
BEGIN
  ...
  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'Validation failed: % apm_alerts reference non-existent assets', invalid_count;
  ELSE
    RAISE NOTICE '✓ Validation 11 passed: All apm_alerts reference valid assets';
  END IF;
END $$;
...
BEGIN
  SELECT COUNT(*) INTO alert_count FROM apm_alerts;
  SELECT COUNT(*) INTO alert_history_count FROM apm_alert_history;
  ...
  RAISE NOTICE '========================================';
  RAISE NOTICE 'apm_alerts: %', alert_count;
