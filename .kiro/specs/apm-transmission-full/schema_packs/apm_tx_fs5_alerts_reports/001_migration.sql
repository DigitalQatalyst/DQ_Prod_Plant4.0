...
-- ============================================================================
-- Table: alerts
-- Purpose: Store real-time alerts from telemetry violations, diagnostic events,
...
CREATE INDEX IF NOT EXISTS idx_alerts_asset_detected
  ON alerts(asset_id, detected_at DESC);

CREATE INDEX IF NOT EXISTS idx_alerts_severity_state_detected
  ON alerts(severity, state, detected_at DESC);

CREATE INDEX IF NOT EXISTS idx_alerts_state_detected
  ON alerts(state, detected_at DESC);

COMMENT ON TABLE alerts IS 'Real-time alerts from telemetry violations, diagnostic events, and failure predictions';
COMMENT ON COLUMN alerts.severity IS 'Alert severity: info, warning, critical, emergency';
COMMENT ON COLUMN alerts.source IS 'Alert source: telemetry, diagnostic, prediction, manual';
COMMENT ON COLUMN alerts.state IS 'Alert state: open, ack (acknowledged), closed';

-- ============================================================================
-- Table: alert_history
-- Purpose: Track alert state transitions and user actions for audit trail
...
CREATE INDEX IF NOT EXISTS idx_alert_history_alert_changed
  ON alert_history(alert_id, changed_at DESC);

COMMENT ON TABLE alert_history IS 'Audit trail of alert state transitions and user actions';
