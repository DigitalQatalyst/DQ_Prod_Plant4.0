-- Seed data for Power Transmission demo tenant and sites
-- This creates the DEWA tenant and 3 sites for the demo
-- Uses clean CTE pattern with UUID generation (no explicit string IDs)

BEGIN;

-- 1) Upsert tenant with a specific UUID that the app can reference
WITH upsert_tenant AS (
  INSERT INTO tenants (id, name, sector, subsector, scenario_tag)
  VALUES ('16043e58-a096-4c02-bea1-9283f75b943c'::uuid, 'DEWA - Transmission', 'power', 'transmission', 'power_transmission_demo_v1')
...
-- 2) Upsert sites using tenant UUID
upsert_sites AS (
