-- ============================================================================
-- APM Transmission FS5: apm_alerts, Reports & Visualisation - Seed Script
-- ============================================================================
-- Feature Set: FS5 - apm_alerts, Reports & Visualisation
-- Purpose: Seed sample data for apm_alerts, alert history, dashboards, reports,
--          and export jobs
...
-- ============================================================================
-- Seed: apm_alerts
-- Purpose: Create sample apm_alerts from various sources
...
DECLARE
  ...
  pred2_id UUID;
BEGIN
  RAISE NOTICE 'Seeding apm_alerts...';
  ...
  -- ========================================================================
  -- apm_alerts from Telemetry Threshold Violations
  -- ========================================================================
  
  -- Critical: TX-001 Top Oil Temperature exceeds critical threshold
  INSERT INTO apm_alerts (
  ...
  -- ========================================================================
  -- apm_alerts from Diagnostic Events
  -- ========================================================================
  ...
  -- ========================================================================
  -- apm_alerts from Failure Predictions
  -- ========================================================================
  ...
  RAISE NOTICE 'Seeded % apm_alerts with variety of severities and states', 
    (SELECT COUNT(*) FROM apm_alerts);
END $;

-- ============================================================================
-- Seed: apm_alert_history
...
BEGIN
  ...
  SELECT id INTO alert_id1 FROM apm_alerts WHERE alert_type = 'temperature_warning' AND state = 'ack' LIMIT 1;
  SELECT id INTO alert_id2 FROM apm_alerts WHERE alert_type = 'load_factor_warning' AND state = 'closed' LIMIT 1;
  SELECT id INTO alert_id3 FROM apm_alerts WHERE alert_type = 'operation_count_milestone' AND state = 'closed' LIMIT 1;
  SELECT id INTO alert_id4 FROM apm_alerts WHERE alert_type = 'failure_prediction_medium' AND state = 'closed' LIMIT 1;
  
  -- Alert history for temperature_warning (open -> ack)
  IF alert_id1 IS NOT NULL THEN
    INSERT INTO apm_alert_history (
  ...
  RAISE NOTICE 'Seeded % alert history records', 
    (SELECT COUNT(*) FROM apm_alert_history);
END $;
...
BEGIN
  ...
  -- Widget: Active Critical apm_alerts
  INSERT INTO dashboard_widgets (
    dashboard_id, widget_type, query_template_ref, position, config
  ) VALUES (
    ...
    '{
      "title": "Active Critical apm_alerts",
...
BEGIN
  ...
  RAISE NOTICE '  - apm_alerts: %', (SELECT COUNT(*) FROM apm_alerts);
  RAISE NOTICE '  - Alert History: %', (SELECT COUNT(*) FROM apm_alert_history);
